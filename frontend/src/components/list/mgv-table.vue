...
export default {
  ...
  methods: {
    initialize() {
      this.headers = [
        {
          label: this.$t("mgv_value").format_letter(),
          field: "degree"
        },
      ...
      this.$axios.get('mgv_request' + "/" + well_id).then(respone => {
    ...
    async save() {
      let hasError = await this.$refs.refMgv.hasError().then(success => {
        return success;
      });
      if (!hasError) {
        this.$q.notify({
